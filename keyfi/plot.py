...
def _set_plot_settings(
    dim: int = 2, figsize: tuple = (7.2, 7.2), view: tuple = (-140, 60)
    ...
    if dim == 2:
        ...
        ax.view_init(*view) #(elevation angle, azimuth angle)
...
def plot_embedding(
    ...
    if cmap_var:
        if cmap_minmax:
            im = ax.scatter(*embedding.T, s=point_size, c=data[cmap_var],  vmin=cmap_minmax[0], vmax=cmap_minmax[1], cmap='inferno')
        else:
            im = ax.scatter(*embedding.T, s=point_size, c=data[cmap_var], cmap='inferno')
